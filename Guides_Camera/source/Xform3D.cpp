...
//
void Xform3D::setShiftVector(double *input)
{
	Xform3D_ShiftSet(this->m_handle, input);
}
/****************************/
/** */
...
void Xform3D::getRotateMat(double *result)
{
	double v[9];
	Xform3D_RotMatGet(this->m_handle,v);
	for (int i = 0; i < 9; i++)
		result[i] = v[i];

}

void Xform3D::setRotateMat(double *input)
{
	Xform3D_RotMatSet(this->m_handle,input);
}